...
class Transactions extends Component
{
    ...
    public function chart(): void
    {
        $query = Sale::selectRaw('SUM(total_amount) as total, SUM(due_amount) as due_amount')
            ...
            ->when(
                ...
                // 📅 MONTHLY
                fn($q) => $q
                    ->selectRaw('EXTRACT(MONTH FROM date) as labels, COUNT(*) as sales')
                ...
                // 📆 YEARLY
                fn($q) => $q
                    ->selectRaw('EXTRACT(YEAR FROM date) as labels, COUNT(*) as sales')
        ...
        $query = Purchase::selectRaw('SUM(total_amount) as total, SUM(due_amount) as due_amount')
            ->when(
                ...
                // 📅 MONTHLY
                fn($q) => $q
                    ->selectRaw('EXTRACT(MONTH FROM date) as labels, COUNT(*) as purchases')
                    ->whereYear('date', date('Y'))
                    ->groupByRaw('EXTRACT(MONTH FROM date)')
                    ->orderBy('labels'),
                ...
                // 📆 YEARLY
                fn($q) => $q
                    ->selectRaw('EXTRACT(YEAR FROM date) as labels, COUNT(*) as purchases')
                    ->groupByRaw('EXTRACT(YEAR FROM date)')
                    ->orderBy('labels')
            )
            ->get()
    ...
    ')
            ...
            // 🔥 filter bulan (PGSQL safe)
            ->whereRaw(
                'EXTRACT(MONTH FROM sale_details.created_at) = ?',
                [now()->month]
            )
    ...
    public function topCustomers()
    {
        ...
        ')
            ...
            // 🔥 group by CUSTOMER (bukan sales.id)
            ->groupBy(
                'sales.customer_id',
                'customers.name',
                'customers.id',
            ...
            // 🔥 order by agregasi (WAJIB di PGSQL)
